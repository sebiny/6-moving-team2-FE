...
## 7. 개인별 주요 작업 내역
    ...
    | --- | --- | --- |
    | 안세빈 (팀장) | 리뷰
    다국어 | - 고객은 기사에 대한 리뷰를 남길 수 있습니다.
    - 다국어 선택이 가능합니다. |
    | 최민경 (부팀장) | 기사님 찾기
    기사님 마이페이지
    AWS | - 기사 리스트를 평점/위치/경력 등의 필터로 조회할 수 있습니다.
    - 기사를 찜 할 수 있습니다.
    - 기사 리스트에서 검색이 가능합니다. |
    | 김단이 | 견적(기사)
    스키마 | - 기사는 고객의 견적 요청서에 대한 견적을 제안할 수 있습니다.
    - 기사는 고객의 지정 견적 요청서를 반려할 수 있습니다.  |
    | 김다은 | 견적(고객) | - 고객은 기사에게 받은 견적서를 조회하고 확정 할 수 있습니다.
    - 고객은 기사에게 받은 견적서를 반려할 수 있습니다.
     |
    | 이지수A | 인증 및 유저, AWS | - 유저 유형별(고객/기사) 로그인, 회원가입이 가능합니다.
    - 유저 유형별로 프로필 등록/수정이 가능합니다.
    - 소셜 로그인이 가능합니다. |
    | 오보람 | 견적요청서 작성(고객) | - 고객은 견적 요청서를 작성할 수 있습니다.
    - 견적 요청서의 주소를 입력할 때 카카오API 활용 해 입력할 수 있습니다. |
    | 황수정 | 알림
    랜딩페이지 | - 실시간으로 알림을 받을 수 있습니다.
    - 이사 당일이 되면 알림을 받을 수 있습니다. |
## 8. 주요 기능 상세
- 인증 시스템
    - 고객 / 기사 회원가입 분리
    - 소셜 로그인
- (고객) 견적 요청서 작성
    - 날짜 선택
    - 주소 조회 및 선택(카카오주소 API)
- (기사) 견적서 작성
    - 받은 견적요청서 조회 및 견적서 전송
- (고객) 견적서 조회
    - 받은 견적서 조회, 견적 확정 가능
- (고객) 기사님 찾기
    - 검색, 정렬
