...
interface RejectedEstimateCardProps {
  request: Request & { estimateAmount?: string; isCompleted?: boolean; status?: string };
}

export default function RejectedEstimateCard({ request }: RejectedEstimateCardProps) {
  const t = useTranslations("MyEstimate");
  const moveTypeKey: MoveType = korToMoveTypeMap[request.moveType] ?? "SMALL";

  return (
    <div className="relative flex w-80 flex-col gap-6 rounded-[20px] bg-white px-5 py-6 shadow-[-2px_-2px_10px_0px_rgba(220,220,220,0.20)] outline outline-offset-[-0.5px] outline-zinc-100 md:w-[588px] md:gap-8 md:px-10 md:py-8">
      {/* 상단 */}
      ...
      <div className="absolute inset-0 z-10">
        <CompletedRejectedCard request={request} />
      </div>
    </div>
  );
}
