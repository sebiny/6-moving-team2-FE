...
interface RejectedEstimateCardListSkeletonProps {
  count?: number; // 로딩 중일 때 보여줄 카드 개수 (기본값 2)
}

export default function RejectedEstimateCardListSkeleton({ count = 2 }: RejectedEstimateCardListSkeletonProps) {
  return (
    <div className="grid grid-cols-1 gap-6 md:gap-8 lg:grid-cols-2 lg:gap-6">
      {Array.from({ length: count }).map((_, idx) => (
        <div
          key={idx}
          className="relative inline-flex w-80 animate-pulse flex-col gap-6 rounded-[20px] bg-gray-200 px-5 py-6 shadow-md outline-[0.5px] outline-offset-[-0.5px] outline-gray-300 md:w-[588px] md:gap-8 md:px-10 md:py-8"
        >
          {/* 상단 */}
